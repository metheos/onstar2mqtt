...
const onstarConfig = {
  ...
  requestPollingIntervalSeconds:
    parseInt(process.env.ONSTAR_POLL_INTERVAL) || 6, // 6 sec default
...
if (!/^[A-HJ-NPR-Z0-9]{17}$/i.test(onstarConfig.vin)) {
  throw new Error(
    "Invalid VIN. Please check the value entered for VIN in ONSTAR_VIN."
  );
}
...
const mqttConfig = {
  ...
  port:
    parseInt(process.env.MQTT_PORT) >= 0
  ...
  ca: process.env.MQTT_CA_FILE
    ? [fs.readFileSync(process.env.MQTT_CA_FILE)]
    : undefined,
  cert: process.env.MQTT_CERT_FILE
    ? fs.readFileSync(process.env.MQTT_CERT_FILE)
    : undefined,
  key: process.env.MQTT_KEY_FILE
    ? fs.readFileSync(process.env.MQTT_KEY_FILE)
    : undefined,
};
...
const getVehicles = async (commands) => {
  ...
  logger.info("Vehicle request status:", {
    status: _.get(vehiclesRes, "status"),
  });
  const vehicles = _.map(
    _.get(vehiclesRes, "response.data.vehicles.vehicle"),
    (v) => new Vehicle(v)
  );
  logger.debug("Vehicle request response:", {
    vehicles: _.map(vehicles, (v) => v.toString()),
  });
  return vehicles;
};
...
const getCurrentVehicle = async (commands) => {
  ...
  const currentVeh = _.find(
    vehicles,
    (v) => v.vin.toLowerCase() === onstarConfig.vin.toLowerCase()
  );
  if (!currentVeh) {
    throw new Error(
      `Configured vehicle VIN ${onstarConfig.vin} not available in account vehicles`
    );
  }
  return currentVeh;
};
...
const connectMQTT = async (availabilityTopic) => {
  const url = `${mqttConfig.tls ? "mqtts" : "mqtt"}://${mqttConfig.host}:${
    mqttConfig.port
  }`;
  ...
  logger.info("Connecting to MQTT:", {
    url,
    config: _.omit(config, "password", "ca", "cert", "key"),
  });
...
const configureMQTT = async (commands, client, mqttHA) => {
  if (!onstarConfig.allowCommands) return;
  ...
  client.on("message", (topic, message) => {
    ...
    const commandStatusTopic = topicArray
      .map((item) => item.topic || item)
      .join("");

    const commandStatusSensorConfig =
      mqttHA.createCommandStatusSensorConfigPayload(
        command,
        mqttConfig.listAllSensorsTogether
      );
    logger.debug("Command Status Sensor Config:", commandStatusSensorConfig);
    const commandStatusSensorTimestampConfig =
      mqttHA.createCommandStatusSensorTimestampConfigPayload(
        command,
        mqttConfig.listAllSensorsTogether
      );
    logger.debug(
      "Command Status Sensor Timestamp Config:",
      commandStatusSensorTimestampConfig
    );

    const commandFn = cmd.bind(commands);
    logger.debug(
      `List of const: Command: ${command}, cmd: ${cmd}, commandFn: ${commandFn.toString()}, options: ${options}`
    );
    if (command === "diagnostics" || command === "enginerpm") {
      ...
      client.publish(
        commandStatusSensorConfig.topic,
        JSON.stringify(commandStatusSensorConfig.payload),
        { retain: true }
      );
      client.publish(
        commandStatusSensorTimestampConfig.topic,
        JSON.stringify(commandStatusSensorTimestampConfig.payload),
        { retain: true }
      );
      client.publish(
        commandStatusTopic,
      ...
      (async () => {
        ...
        logger.info("Diagnostic request status", {
          status: _.get(statsRes, "status"),
        });
        logger.debug(
          "Diagnostic Response Body from Command",
          statsRes.response.data.commandResponse.body.diagnosticResponse
        );
        // Make sure the response is always an array
        const diagnosticResponses = _.get(
          statsRes,
          "response.data.commandResponse.body.diagnosticResponse"
        );
        const diagArray = Array.isArray(diagnosticResponses)
          ? diagnosticResponses
          : [diagnosticResponses];
        const stats = _.map(diagArray, (d, index) => {
          logger.debug("Diagnostic Array", { ...d, number: index + 1 });
          return new Diagnostic({ ...d, number: index + 1 });
        });
        logger.debug("stats", stats);
        logger.debug("Diagnostic request response:", {
          stats: _.map(stats, (s) => s.toString()),
        });
        ...
        if (e instanceof Error) {
          ...
          logger.error(
            `Command Status Topic for Errored Command: ${commandStatusTopic}`
          );
          client.publish(
            commandStatusTopic,
            JSON.stringify({
      ...
      logger.debug(
        `Command sent: Command: ${command}, Request: ${JSON.stringify(request)}`
      );
        ...
        .then((data) => {
          ...
          if (responseData) {
            ...
            const location = _.get(
              data,
              "response.data.commandResponse.body.location"
            );
            const speed = _.get(
              data,
              "response.data.commandResponse.body.speed"
            );
            const direction = _.get(
              data,
              "response.data.commandResponse.body.direction"
            );
            if (location) {
              const topic = mqttHA.getStateTopic({ name: command });
              const deviceTrackerConfigTopic =
                mqttHA.getDeviceTrackerConfigTopic();
              ...
              client
                .publish(
                  deviceTrackerConfigTopic,
                  JSON.stringify(deviceTrackerConfig),
                  { retain: true }
                )
                .then(() => {
                  logger.warn(
                    `Published device_tracker config to topic: ${deviceTrackerConfigTopic}`
                  );
...
(async () => {
  try {
    ...
    client
      .publish(availTopic, "true", { retain: true })
    ...
    const run = async () => {
      ...
      const pollingStatusTopicState = topicArray
        .map((item) => item.topic || item)
        .join("");
      logger.info(`pollingStatusTopicState: ${pollingStatusTopicState}`);

      const pollingStatusMessagePayload =
        mqttHA.createPollingStatusMessageSensorConfigPayload(
      ...
      const pollingStatusCodePayload =
        mqttHA.createPollingStatusCodeSensorConfigPayload(
          pollingStatusTopicState,
          mqttConfig.listAllSensorsTogether
        );
      logger.debug("pollingStatusCodePayload:", pollingStatusCodePayload);
      const pollingStatusMessageTimestampPayload =
        mqttHA.createPollingStatusTimestampSensorConfigPayload(
          pollingStatusTopicState,
          mqttConfig.listAllSensorsTogether
        );
      logger.debug(
        "pollingStatusMessageTimestampPayload:",
        pollingStatusMessageTimestampPayload
      );
      ...
      if (!buttonConfigsPublished) {
        client.publish(
          pollingStatusMessagePayload.topic,
          JSON.stringify(pollingStatusMessagePayload.payload),
          { retain: true }
        );
        client.publish(
          pollingStatusCodePayload.topic,
          JSON.stringify(pollingStatusCodePayload.payload),
          { retain: true }
        );
        client.publish(
          pollingStatusMessageTimestampPayload.topic,
          JSON.stringify(pollingStatusMessageTimestampPayload.payload),
          { retain: true }
        );
      ...
      let topicArrayTF;
      if (!mqttConfig.pollingStatusTopic) {
        topicArrayTF = _.concat(
          mqttHA.getPollingStatusTopic(),
          "/",
          "lastpollsuccessful"
        );
      } else {
        topicArrayTF = _.concat(
          mqttConfig.pollingStatusTopic,
          "/",
          "lastpollsuccessful"
        );
      }
      const pollingStatusTopicTF = topicArrayTF
        .map((item) => item.topic || item)
        .join("");
      logger.info(`pollingStatusTopicTF, ${pollingStatusTopicTF}`);

      if (!buttonConfigsPublished) {
        const pollingStatusTFPayload =
          mqttHA.createPollingStatusTFSensorConfigPayload(
            pollingStatusTopicTF,
            mqttConfig.listAllSensorsTogether
          );
        logger.debug("pollingStatusTFPayload:", pollingStatusTFPayload);
        client.publish(
          pollingStatusTFPayload.topic,
          JSON.stringify(pollingStatusTFPayload.payload),
          { retain: true }
        );
      ...
      if (!buttonConfigsPublished) {
        const sensors = [
          ...
          {
            name: "tire_pressure",
            component: "tire_pressure_lf_message",
            icon: "mdi:car-tire-alert",
          },
          {
            name: "tire_pressure",
            component: "tire_pressure_lr_message",
            icon: "mdi:car-tire-alert",
          },
          {
            name: "tire_pressure",
            component: "tire_pressure_rf_message",
            icon: "mdi:car-tire-alert",
          },
          {
            name: "tire_pressure",
            component: "tire_pressure_rr_message",
            icon: "mdi:car-tire-alert",
          },
        ];

        for (let sensor of sensors) {
          const sensorMessagePayload = mqttHA.createSensorMessageConfigPayload(
            sensor.name,
            sensor.component,
            sensor.icon
          );
          logger.debug(
            `Sensor Message Payload: ${sensor.name}, ${sensor.component}`,
            sensorMessagePayload
          );
          client.publish(
            sensorMessagePayload.topic,
            JSON.stringify(sensorMessagePayload.payload),
            { retain: true }
          );
        }
      ...
      function publishButtonConfigs() {
        ...
        if (!buttonConfigsPublished) {
          ...
          if (mqttConfig.listAllSensorsTogether === true) {
            tasks = [mqttHA.createButtonConfigPayload(v)];
          } else {
            tasks = [
          ...
          tasks.forEach(({ buttonConfigs, configPayloads }, taskIndex) => {
            ...
            buttonConfigs.forEach((buttonConfig, index) => {
              ...
              const buttonType =
                taskIndex === 0 ? "Button" : "Button for Command Status";
              logger.warn(
                `${buttonType} Config Topic: ${JSON.stringify(buttonConfig)}`
              );
              logger.debug(
                `${buttonType} Config Payload: ${JSON.stringify(configPayload)}`
              );

              // Publish configPayload as the payload to the respective MQTT topic
              logger.debug(
                `Publishing ${buttonType} Config: ${buttonConfig} Payload: ${JSON.stringify(
                  configPayload
                )}`
              );
              client.publish(buttonConfig, JSON.stringify(configPayload), {
                retain: true,
              });
            });
          });
    ...
    const refreshIntervalCurrentValTopic =
      mqttHA.getRefreshIntervalCurrentValTopic();
    logger.info(`refreshIntervalTopic: ${refreshIntervalTopic}`);
    logger.info(
      `refreshIntervalCurrentValTopic: ${refreshIntervalCurrentValTopic}`
    );

    if (!refreshIntervalConfigPublished) {
      const pollingRefreshIntervalPayload =
        mqttHA.createPollingRefreshIntervalSensorConfigPayload(
          refreshIntervalCurrentValTopic,
          mqttConfig.listAllSensorsTogether
        );
      logger.debug(
        "pollingRefreshIntervalSensorConfigPayload:",
        pollingRefreshIntervalPayload
      );
      client.publish(
        pollingRefreshIntervalPayload.topic,
        JSON.stringify(pollingRefreshIntervalPayload.payload),
        { retain: true }
      );
    ...
    client.publish(
      refreshIntervalCurrentValTopic,
      onstarConfig.refreshInterval.toString(),
      { retain: true }
    );
